...
import yaml
import sys
from rclpy.qos import qos_profile_sensor_data  # good for IMU, camera, etc.
...
class Subs(object):
    ...
    def __init__(self):
        ...
        self.node = rclpy.create_node('get_mav_data')
        # Dict with all data
        self.data = {}
    ...
    def set_data(self, path, value={}):
        ...
        if value is not {}:
            current_level.update(yaml.load(str(value), Loader=yaml.SafeLoader))
...
if __name__ == '__main__':
    ...
    def print_voltage():
        try:
            sub.node.get_logger().info(sub.get_data()['mavros']['battery']['voltage'])
        except Exception as error:
            print(error)

    while not sub.node.shutdown():
        print_voltage()
        time.sleep(1)